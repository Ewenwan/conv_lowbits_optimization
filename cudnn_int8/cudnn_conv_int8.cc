...
struct TrainingContext {
  ...
  size_t SetFwdConvolutionTensors(ConvolutionLayer& conv,
    ...
    // * CUDNN_DATA_FLOAT -> CUDNN_DATA_INT8x4, and 
    // * CUDNN_TENSOR_NCHW -> CUDNN_TENSOR_NCHW_VECT_C
    ...
    // Set output tensor (Changed CUDNN_DATA_FLOAT to CUDNN_DATA_INT8x4, following the manual)
    checkCUDNN(cudnnSetTensor4dDescriptor(
        dstTensorDesc, CUDNN_TENSOR_NCHW_VECT_C, CUDNN_DATA_INT8x4, n, c, h, w));
  ...
  void ForwardPropagation(char* data,
                          char* conv1,
                          char* pconv1,
                          void* workspace) {
    int alpha = 1.0f;
...
int main(int argc,char *argv[ ]) {
  ...
  for (auto&& iter : conv1.pconv) {
    iter = static_cast<char>(dconv1(g));
  }

  // Initailize input image (batch size = 1)
  std::vector<char> img_char(1 * width * height * channels);
  for (auto&& iter : img_char) {
    iter = static_cast<char>(dconv1(g));
  }
  ...
  checkCudaErrors(cudaMalloc(&d_data,
                             sizeof(char) * context.m_batchSize * channels *
                                 height * width));
  checkCudaErrors(cudaMalloc(&d_conv1,
                             sizeof(char) * context.m_batchSize *
                                 conv1.out_channels * conv1.out_height *
                                 conv1.out_width));
  checkCudaErrors(cudaMemcpyAsync(d_data,
                                  &img_char[0],
                                  sizeof(char) * 1 * channels * width * height,
                                  cudaMemcpyHostToDevice));

  // Allocate kernel on GPU
  char* d_pconv1;
  checkCudaErrors(cudaMalloc(&d_pconv1, sizeof(char) * conv1.pconv.size()));
  checkCudaErrors(cudaMemcpyAsync(d_pconv1,
                                  &conv1.pconv[0],
                                  sizeof(char) * conv1.pconv.size(),
                                  cudaMemcpyHostToDevice));
